...
\begin{frame}[label={sec:orgc0c76ce}]{AIGORA}
...
\label{sec:org81c869b}
\subsection{¿Qué es el control de versiones?}
\label{sec:org4a38fc3}

\begin{frame}[plain,label={sec:org74468d7}]{}
\begin{center}
...
\begin{frame}[plain,label={sec:org200d286}]{}
\begin{center}
...
\begin{frame}[plain,label={sec:org0634601}]{}
\begin{center}
...
\begin{frame}[label={sec:orgf71cee4}]{¿Qué es el control de versiones y por qué debería importarte?}
\begin{quote}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:orgd534e60}]{¿Qué es el control de versiones y por qué debería importarte?}
\begin{quote}
...
personas.\footnote{\url{https://swcarpentry.github.io/git-novice/}\label{org729ab4f}}
\end{quote}
...
\begin{frame}[label={sec:org518f27a}]{Git es un Sistema de Control de Versiones}
...
\begin{frame}[label={sec:org62649e3}]{Git es un Sistema de Control de Versiones}
...
\begin{frame}[label={sec:org80b6daa}]{Los estados de Git}
\begin{itemize}
...
\begin{frame}[fragile,label={sec:orgec2a32f}]{¿Qué es GitHub?}
 \begin{itemize}
...
\label{sec:org6a2bff8}

\subsection{Primeros Pasos}
\label{sec:org62cb86e}
\begin{frame}[label={sec:org0605ed3}]{Creación de una cuenta en GitHub}
...
\begin{frame}[label={sec:orgd3002bf}]{Instalación de GitHub Desktop}
...
\begin{frame}[label={sec:org7807503}]{Método 1: de remoto a local}
...
\begin{frame}[fragile,label={sec:orgfd70464}]{Cambios en la copia local}
 \begin{center}
\includegraphics[height=0.4\textheight]{figs/git_estados.png}
\end{center}
...
\end{center}
\end{frame}
\begin{frame}[fragile,label={sec:org974ef04}]{Cambios en la copia local}
 \begin{center}
\includegraphics[height=0.4\textheight]{figs/git_estados.png}
\end{center}
...
\end{center}
\end{frame}
\begin{frame}[fragile,label={sec:orgd98d5e4}]{Consejos para \texttt{commit}}
 \begin{itemize}
...
\begin{frame}[fragile,label={sec:org1af8d60}]{Recibir cambios de un repositorio remoto}
...
\end{block}
\end{frame}
\begin{frame}[label={sec:org6b99cf8}]{Resumen}
\begin{enumerate}
...
\label{sec:orgda6e914}
\subsection{Ramas}
\label{sec:org292a62d}

\begin{frame}[fragile,label={sec:orgf159b99}]{Rama \texttt{master}}
 \begin{center}
...
\begin{frame}[label={sec:org9ad0c24}]{Ramas para facilitar la colaboración}
\begin{center}
\includegraphics[width=.9\linewidth]{figs/branching.png}
\end{center}
...
\begin{frame}[fragile,label={sec:org420c29b}]{Crear una nueva rama}
 \begin{itemize}
...
\begin{frame}[label={sec:org33ef604}]{Combinación de código}
\begin{center}
\includegraphics[width=.9\linewidth]{figs/branching.png}
\end{center}
...
\begin{frame}[label={sec:org9e99908}]{Combinación de código}
\begin{center}
\includegraphics[width=.9\linewidth]{figs/branching.png}
\end{center}
...
\begin{frame}[label={sec:orgc73555c}]{Resolución de conflictos}
...
\end{frame}
\begin{frame}[fragile,label={sec:org3298ebe}]{Consejos}
 \begin{itemize}
...
\end{itemize}
\end{frame}
\begin{frame}[fragile,label={sec:org66f42b6}]{}
...
\label{sec:org7438347}

\begin{frame}[label={sec:org0ee0b90}]{Issues}
...
\begin{frame}[label={sec:org8254e69}]{Estructura de una issue}
...
\begin{frame}[label={sec:org9765541}]{Contenido de una issue}
\begin{itemize}
...
\begin{frame}[fragile,label={sec:org8fef376}]{}
 \begin{block}{Ejercicio}
\begin{itemize}
...
\label{sec:org38e20bc}

\begin{frame}[label={sec:org1c7d34f}]{Insights}
...
\begin{frame}[label={sec:org6357d53}]{Contribución de los integrantes del equipo}
\begin{center}
\includegraphics[width=.9\linewidth]{figs/repo_contributors_specific_graph.png}
\end{center}
\end{frame}

\begin{frame}[label={sec:org4f2de90}]{Estructura de ramas de un repositorio}
\begin{center}
...
\begin{frame}[fragile,label={sec:orge60010b}]{GitHub Classroom necesita una Organización}
 \begin{itemize}
...
\begin{frame}[fragile,label={sec:org718f970}]{Dentro de una organización se pueden definir equipos}
 \begin{itemize}
\item Dentro de una \texttt{organization} pueden coexistir diferentes equipos (\texttt{Team})

\item Se pueden asignar diferentes permisos de acceso y escritura a cada equipo, o a conjuntos de equipos.

\item Ejemplo:

\url{https://github.com/orgs/aigora/teams}

\item Más información en el \href{https://github.blog/2010-06-29-introducing-organizations/}{blog de GitHub}.
\end{itemize}
\end{frame}

\begin{frame}[fragile,label={sec:orgcfb3594}]{Configuración de aulas}
 \begin{itemize}
\item Una única cuenta \texttt{organization} puede dar cabida a múltiples aulas (\texttt{classrooms}): 
...
\item Esta correspondencia se puede facilitar mediante el \texttt{classroom roster}\footnote{\href{https://docs.github.com/es/free-pro-team@latest/education/manage-coursework-with-github-classroom/manage-classrooms\#creating-a-roster-for-your-classroom}{Creating a roster for your classroom (GitHub Docs)}}.
\end{itemize}
\end{frame}

\begin{frame}[fragile,label={sec:orga1b8609}]{Asignación de tareas}
 \begin{itemize}
\item Dentro de cada \texttt{classroom} se pueden incluir múltiples tareas (\texttt{assignments}).
\item Un \texttt{assignment} puede ser individual o grupal.
\item Los grupos se pueden definir antes por el profesor, o por los estudiantes en el momento de aceptar la tarea.
\end{itemize}
\end{frame}


\begin{frame}[fragile,label={sec:orgdc90ecf}]{Cómo hemos usado GitHub Classroom en AIGORA}
...
\end{block}
\end{frame}
\begin{frame}[fragile,label={sec:orgb7a95bd}]{Cómo hemos usado GitHub Classroom en AIGORA}
...
\end{block}
\end{frame}
\begin{frame}[fragile,label={sec:orge9f0d46}]{Cómo hemos usado GitHub Classroom en AIGORA}
...
\label{sec:orgbc9475f}

\begin{frame}[fragile,label={sec:org3a91c8c}]{Página web de \alert{proyecto}}
...
\begin{frame}[fragile,label={sec:org13a37d4}]{Página web de \alert{proyecto}}
 \url{https://<username>.github.io/<repository>}
...
\begin{itemize}
\item En la página del repositorio: \boxed{Settings > GitHub\ Pages > Source > docs\ folder}
\end{itemize}
\end{block}
\end{frame}

\begin{frame}[label={sec:org2c68485}]{Ejemplo de web de \alert{proyecto}}
\begin{itemize}
...
\end{itemize}
\begin{center}
\includegraphics[height=0.5\textheight]{figs/captura_bookvis.png}
\end{center}
\end{frame}


\begin{frame}[fragile,label={sec:org86bce64}]{Página web de \alert{usuario u organización}}
 \begin{enumerate}
...
\begin{frame}[label={sec:org4ea7c19}]{Ejemplo de web de \alert{organización}}
\begin{itemize}
