...
- No olvides inicializar y cumplimentar el *=README.md=*. Para el formato veáse [[https://help.github.com/articles/basic-writing-and-formatting-syntax/][Formatting syntax]].
...
1. Modifica los ficheros de la copia local.
2. Añade los cambios realizados a la siguiente "instantánea" del repositorio (=git add=) [[file:figs/git_add.png]]
3. Confirma los cambios, escribiendo un resumen de lo realizado (=git commit=)
...
** Resumen de lo hecho en AIGORA
- Se pueden hacer varias aulas (classrooms con una única organization de GitHub)
- Usamos GitHub Classroom únicamente para configurar los Teams.
- Subimos un roster a cada aula (lista de correos alumnos.upm.es)
- En cada classroom abrimos una Group Assignment denominada "registro_estudiantes". Le ponemos un identificador al "Group" igual al grupo de matriculación. Este Group queda almacenado en GitHub classroom para futuros assignment, pero no crea un Team en GitHub.
- Abrimos el enlace del assignment con nuestro usuario (podemos hacer skip cuando nos pide que enlacemos a algún miembro del roster), y creamos un nuevo Team, que se debe llamar igual que el grupo de matriculación. Este paso crea un nuevo Team en la organization de Github. Lo podemos usar para dar permisos.
- Ahora le enviamos el enlace del assignment a los alumnos para que enlacen su usuario de GitHub a su correo incluido en el roster. 
- Deben elegir el Team que hemos definido nosotros, sin crear uno nuevo. A partir de este punto, están incluidos en este Team, y por tanto adquieren los mismos permisos y repositorios que el Team.
- Para el trabajo en equipo hacemos una group assignment, pero les dejamos libertad a la hora de definir los Teams.
