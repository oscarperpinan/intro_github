...
- Añade los cambios realizados a la siguiente "instantánea" del repositorio: 
#+begin_center
=git add=
#+end_center
- Confirma los cambios (escribiendo un resumen de lo realizado): 
#+begin_center
=git commit=
#+end_center
...
1. =git fetch=, obtener datos recientes del repositorio.
2. =git merge=, combinarlos con la copia local.
