...
*** 
**** Ejercicio 
1. Crea un nuevo repositorio remoto y haz la clonación del mismo para tener una copia local.
2. Crea un nuevo repositorio local y publícalo en remoto. 

En ambos casos, no olvides elegir la licencia, generar un =README= y un =.gitignore=.
** Flujo de Trabajo
*** 
[[file:figs/git_estados.png]]
...
** COMMENT Recuperación de cambios
*** Recordatorio
#+begin_quote
El control de versiones es un sistema que *registra los cambios*
realizados sobre un archivo o conjunto de archivos a lo largo del
...
*** =git checkout=
- Recuperar el fichero =filename= del último =commit= (es decir, descartar cambios realizados recientemente):
...
- Una única cuenta =organization= puede dar cabida a múltiples aulas (=classrooms=): https://classroom.github.com/classrooms/
